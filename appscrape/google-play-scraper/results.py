...
#connect to db 
try:
    sqliteConnection = sqlite3.connect('appresults.db')
    ...
    for row in result:
        ...
        count = cursor.execute('''INSERT INTO app ('appID', 'title', 'rating', 'genre', 'price', 'developer', 'downloads') VALUES (?,?,?,?,?,?,?)''', arr)
        sqliteConnection.commit()
        print("Record inserted successfully into app table ", cursor.rowcount)
    cursor.close()
